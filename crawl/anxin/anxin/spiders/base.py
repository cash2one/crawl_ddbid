...
class AnxinSpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        item['amount'] = sel.xpath('//div[@class=\"item_content\"]/text()').extract()[0]
        item['min_amount'] = ''
        item['income_rate'] = sel.xpath('//div[@class=\"item_content\"]/text()').extract()[1]

        term1 = sel.xpath('//div[@class=\"item_content\"]/span/text()').extract()[0]
        term2 = sel.xpath('//div[@class=\"item_content\"]/span/text()').extract()[1]
        item['term'] = term1+term2
        item['area'] = ''
        item['transfer_claim'] = ''
        item['repay_type'] = sel.xpath('//div[@class=\"borrow_kind_c\"]/span/text()').extract()[0]
        item['reward'] = ''

        item['protect_mode'] = ''
        item['description'] = sel.xpath('//p[@class=\"detail_info_p\"]/text()').extract()[0].strip()
        item['process'] = sel.xpath('//div[@class=\"progress_bar\"]/text()').extract()[0]
